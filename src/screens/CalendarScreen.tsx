...
const fromFsTimestamp = (v: any): string | undefined => {
...
function monthRange(isoYmd: string) {
...
export default function CalendarRecordScreen() {
      ...
      try {
        ...
        (res.data || []).forEach((d: any) => {
          const item: Cloth = {
            ...
            thumbnail: toAbs(d.thumbnail ?? d.image_url ?? ''),
  ...
  const clothesForPick = useMemo(() =>
    Object.values(clothes).filter(c => {
    ...
    <GestureHandlerRootView style={styles.container}>
      ...
      <View style={styles.listHeader}>
        <Text style={styles.listHeaderText}>
          {selected}의 기록 · {listForSelectedDay.length}개
        </Text>
...
const styles = StyleSheet.create({
  ...
  headerSpacer: { paddingTop: 60, paddingBottom: 8, alignItems: 'center', justifyContent: 'center' },
  backButtonAbsolute: { position: 'absolute', left: 16, top: 60, zIndex: 1 },
  ...
  modalCard: {
    width: '86%', backgroundColor: '#fff', borderRadius: 16,
